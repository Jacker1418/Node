...
static void open_GPIO(void* in_instance, uint8_t in_action);
static uint32_t write_GPIO(void* in_instance, void* in_data, uint32_t in_length);
static ret_code_t read_GPIO(void* in_instance, void* out_data, uint32_t* out_length);
static ret_code_t ioctrl_GPIO(void* in_instance, uint8_t in_option, uint8_t* out_result);
...
{
    out_instance->instance = NRF_GPIO;
...
void open_GPIO(void* in_instance, uint8_t in_action)
{
...
uint32_t write_GPIO(void* in_instance, void* in_data, uint32_t in_length)
{
    uint32_t result = in_length;

    uint32_t* pos_pin  = (uint32_t *)in_instance;

    uint8_t* st_pin = (uint8_t *) in_data;

    nrf_gpio_pin_write(*pos_pin, *st_pin);

    return result;
}

ret_code_t read_GPIO(void* in_instance, void* out_data, uint32_t* out_length)
{
    ...
    *data = nrf_gpio_pin_input_get(*pos_pin);
    *out_length = 1;

    return result;
}

ret_code_t ioctrl_GPIO(void* in_instance, uint8_t in_option, uint8_t* out_result)
{
    ret_code_t result = NRF_SUCCESS;
